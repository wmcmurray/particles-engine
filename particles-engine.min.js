...
Particle.prototype.place=function(x,y,z){TweenMax.to(this.view,0,{x:-(jQuery(this.view).outerWidth()*0.5)+x,y:-(jQuery(this.view).outerHeight()*0.5)+y,z:z});jQuery(this.view).css({zIndex:z});return this};Particle.prototype.resize=function(size){this.params.size=size;this.view.style.width=size+"px";this.view.style.height=size+"px";if(this.params.round)jQuery(this.view).css({borderRadius:this.params.size*0.5+"px"});return this};
Particle.prototype.applyForce=function(vector,deltatime,multiplier){this.vel.apply(vector,deltatime,multiplier?multiplier:1);return this};Particle.prototype.update=function(deltatime){if(this.params.solid)if(this.pos.z<0&&this.vel.z<0)this.vel.z=-this.vel.z*0.75;this.vel.multiply(this.params.friction);this.pos.apply(this.vel,deltatime);this.place(this.pos.x,this.pos.y,this.pos.z);if(this.params.lifetime&&this.params.fade)jQuery(this.view).css({opacity:1-this.getLifeSpent()});return this};
Particle.prototype.kill=function(){this.parent.removeChild(this.view);this.killed=true;return this};function ParticlesEmitter(parent,params){this.params={relative:false,pps:5,limit:0,particles:{},autoStart:false,from:"center",spread:500};for(var i in params)this.params[i]=params[i];this.parent=parent?jQuery(parent)[0]:document.body;this.container=this.params.relative?jQuery(this.parent)[0]:jQuery(".particles-engine-display")[0];this.pos=new Vector(0,0,0);this.vel=new Vector(0,0,0);this.width=0;this.height=0;this.counter=0;this.loopInterval;this.particles=new Array;this.init()}
ParticlesEmitter.prototype.init=function(){jQuery(this.container).css({transformStyle:"preserve-3d"});this.refreshPos();if(!this.parent.emitters)this.parent.emitters=new Array;this.parent.emitters.push(this);if(this.params.autoStart)this.start()};ParticlesEmitter.prototype.start=function(){this.stop();this.loopInterval=setInterval(jQuery.proxy(this.createParticle,this),1E3/this.params.pps);jQuery(this.parent).addClass("emitting");return this};
ParticlesEmitter.prototype.stop=function(){if(this.loopInterval){clearInterval(this.loopInterval);this.loopInterval=false;jQuery(this.parent).removeClass("emitting")}return this};ParticlesEmitter.prototype.reset=function(){this.counter=0;return this};ParticlesEmitter.prototype.burst=function(amount){for(var i=0;i<amount;i++)this.createParticle();return this};ParticlesEmitter.prototype.toogle=function(){if(this.loopInterval)this.stop();else this.start();return this};
ParticlesEmitter.prototype.refreshPos=function(){var p=jQuery(this.parent);this.width=p.width();this.height=p.height();if(!this.params.relative){var o=p.offset();this.pos.x=o.left+this.width*0.5;this.pos.y=o.top+this.height*0.5;this.pos.z=0}else{this.pos.x=this.width*0.5;this.pos.y=this.height*0.5;this.pos.z=0}var now=(new Date).getTime();if(this.lastPosRefreshTime){var deltaTime=now-this.lastPosRefreshTime;var pos=new Vector(this.pos);pos.substractVector(this.lastPosRefreshPos).multiply(1E3/deltaTime);
this.vel=pos}this.lastPosRefreshTime=now;this.lastPosRefreshPos=new Vector(this.pos)};
ParticlesEmitter.prototype.createParticle=function(){if(this.params.limit&&++this.counter>=this.params.limit){this.stop();return false}else{this.refreshPos();var p=new Particle(this.params.particles);p.pos.apply(this.pos);var values=this.params.from.split(" ");if(values.length<2)switch(values[0]){case "top":case "bottom":values.push(values[0]);values[0]="random";break;case "left":case "right":values.push("random");break;default:values.push(values[0]);break}if(values[0]=="left")values[0]=0;else if(values[0]==
"right")values[0]=1;else if(values[0]=="center")values[0]=0.5;else if(values[0]=="random")values[0]=Math.random();else values[0]=Number(values[0]);if(values[1]=="top")values[1]=0;else if(values[1]=="bottom")values[1]=1;else if(values[1]=="center")values[1]=0.5;else if(values[1]=="random")values[1]=Math.random();else values[1]=Number(values[1]);var newPos=new Vector(this.width*(values[0]-0.5),this.height*(values[1]-0.5),0);p.pos.apply(newPos);if(p.params.inheritVelocity)p.vel.apply(this.vel);if(this.params.spread){var m=
this.params.spread;var x=Math.random()*m-m/2;var y=Math.random()*m-m/2;var z=Math.random()*m-m/2;p.vel.apply(new Vector(x,y,z))}p.place(p.pos.x,p.pos.y,p.pos.z);p.inject(this.container);this.particles.push(p);return p}};function ParticlesEngine(params){this.params={fps:30,container:false,gravity:new Vector(0,300,0),perspective:600};for(var i in params)this.params[i]=params[i];this.loopInterval;this.lastFrame;this.emitters=new Array;this.init()}
